...
if len(plot_data) == 0:
    ...
    if log_y:
        # Log scale can't show negative values – fall back to default auto-range.
        ...
        x_abs_max = float(plot_data["Signed Weight"].abs().max() or 0)
        y_abs_max = float(plot_data["Performance % (plot)"].abs().max() or 0)
        x_pad = x_abs_max * 1.08 if x_abs_max > 0 else 1
        y_pad = y_abs_max * 1.08 if y_abs_max > 0 else 1

        fig.update_xaxes(showgrid=True, gridcolor="rgba(128,128,128,0.18)",
                         zeroline=False, autorange=False, range=[-x_pad, x_pad])
        fig.update_yaxes(showgrid=True, gridcolor="rgba(128,128,128,0.18)",
                         zeroline=False, autorange=False, range=[-y_pad, y_pad])
