...
# AFTER `RENAMES` has been applied. Add entries here only when the Current
# Portfolio sheet labels a company differently from the Trades sheet
# (e.g. capitalisation differences or HK-line vs. parent-company naming).
# Matching is case-insensitive on both sides.
NAME_ALIASES: dict[str, str] = {
    # Empty for now – Beone is consistently named in all sheets, so a simple
    # RENAMES entry handles it. Re-populate here if a future Current-Portfolio
    # entry doesn't match the Trades sheet.
}
...
def build_pickle(input_xlsx: str | Path, output_pkl: str | Path = "data.pkl") -> dict:
    ...
    try:
        ...
        # Apply rebrand renames to current-portfolio names too, so they
        # line up with the (already-renamed) trades and prices.
        if RENAMES:
            current_raw = [RENAMES.get(n, n) for n in current_raw]
