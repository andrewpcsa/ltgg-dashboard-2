...
floor_y_at_neg100 = st.sidebar.checkbox(
    "Floor Y axis at -100%", value=False,
    help="A stock can't fall more than 100% (you can only lose what you invested). "
         "Toggling this on caps the negative side of the Y axis at -100% instead "
         "of mirroring the full positive range, so the chart isn't dominated by "
         "impossible-loss whitespace when a big winner stretches the scale.",
)
...
if len(plot_data) == 0:
    ...
    if log_y:
        ...
        # Decide Y-axis bounds. Default = symmetric around 0. With the
        # "floor at -100%" toggle on, bottom is fixed at -100 (the
        # impossible-loss line) and the top follows the positive data extent.
        if floor_y_at_neg100:
            y_min = -100.0
            pos_max = float(plot_data["Performance % (plot)"].max() or 0)
            y_max = max(pos_max * 1.08, 10.0)   # always show some positive headroom
        else:
            y_min = -y_abs
            y_max = y_abs
